...
do
  TRANSLATION_FILE="vector_correction/i18n/$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "vector_correction/i18n/"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    ${PYLUPDATE} -noobsolete ${PYTHON_FILES} -ts vector_correction/i18n/${LOCALE}.ts
  done
else
